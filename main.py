...
def randomize_positions():
    global kurir_pos, source_pos, dest_pos, sudah_ambil, paket_diterima
...
def move_kurir(kurir_pos, kurir_angle, speed):
    ...
    # Pastikan kurir tetap di jalan yang bisa dilalui
    if is_road(get_pixel_color(new_pos)):
        return new_pos
    
    # Coba untuk mencari posisi yang lebih "tengah" di sekitar kurir
    best_pos = kurir_pos
    best_distance = float('inf')
    
    # Cek beberapa posisi sekitar (misalnya ke kiri, kanan, atas, bawah)
    for angle_offset in [-45, 0, 45]:  # Coba geser sedikit ke kiri atau kanan, atau tetap lurus
        new_angle = kurir_angle + angle_offset
        dx = speed * np.cos(np.radians(new_angle))
        dy = -speed * np.sin(np.radians(new_angle))
        test_pos = [kurir_pos[0] + dx, kurir_pos[1] + dy]
        
        # Periksa apakah posisi yang diuji adalah jalur yang dapat dilalui
        if is_road(get_pixel_color(test_pos)):
            # Hitung jarak dari posisi baru ke posisi tengah jalur
            distance_to_center = np.linalg.norm(np.array(test_pos) - np.array(kurir_pos))
            if distance_to_center < best_distance:
                best_pos = test_pos
                best_distance = distance_to_center
    
    return best_pos  # Kembalikan posisi yang lebih baik dan lebih "tengah"
...
def main():
    global kurir_pos, kurir_angle, is_moving, sudah_ambil, use_manual_control, kurir_visible, kurir_speed, paket_diterima
    ...
    while running:
        ...
        if paket_diterima:
            teks_diterima = font.render("Paket Diterima!", True, BLUE)
            SCREEN.blit(teks_diterima, (750, 30))
